...
export class TicketService {
  ...
  async getTickets(query, user) {
    ...
    const [data, total] = await Promise.all([
      this.ticketRepository.findAllTickets(filter, {
        skip,
        limit: currentLimit,
        sort: sortObject
      }),
      this.ticketRepository.countTickets(filter)
    ]);
  ...
  async assertCanManage(ticket, user) {
    ...
    const isOwner = ticket.user?.id
      ? ticket.user.id.toString() === user.id.toString()
      : ticket.user.toString() === user.id.toString();

    if (!isOwner) {
      throw businessError(
        "No tenés permisos para modificar este ticket",
        403
      );
    }
  }
